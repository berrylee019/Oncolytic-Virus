...
def generate_blog_post_groq(data_df):
    ...
    # prompt 수정 예시
    prompt = f"""
    당신은 한국의 1등 의학 전문 블로거입니다. 
    아래 데이터를 바탕으로 글을 쓰되 다음 규칙을 엄격히 지키세요:
    1. '일제삼' 같은 어색한 번역투 금지.
    2. 영문 약어(effective dose 등)는 반드시 쉬운 한글(최적 용량)로 번역.
    3. 중간중간 이모지를 적절히 섞어 가독성을 높일 것.
    4. 각 임상마다 '환자들에게 주는 의미'를 한 줄씩 요약할 것.
    """
